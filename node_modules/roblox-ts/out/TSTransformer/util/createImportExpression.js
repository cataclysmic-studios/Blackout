...
function getImportParts(state, sourceFile, moduleSpecifier, moduleOutPath, moduleRbxPath) {
...
function createImportExpression(state, sourceFile, moduleSpecifier) {
    ...
    const isInsideNodeModules = typescript_1.default.isInsideNodeModules(virtualPath);
    const moduleOutPath = isInsideNodeModules
        ? state.pathTranslator.getImportPath((_a = state.nodeModulesPathMapping.get((0, getCanonicalFileName_1.getCanonicalFileName)(path_1.default.normalize(virtualPath)))) !== null && _a !== void 0 ? _a : virtualPath, true)
        : state.pathTranslator.getImportPath(virtualPath);
    const parts = new Array();
    parts.push(luau_ast_1.default.globals.script);
