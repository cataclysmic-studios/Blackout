    ...
    if (typescript_1.default.isVariableDeclarationList(initializer) &&
        ...
        for (let i = 0; i < tupleType.elementFlags.length; i++) {
            ...
            if (tupleType.labeledElementDeclarations) {
                ...
                if (typescript_1.default.isIdentifier(label.name) && luau_ast_1.default.isValidIdentifier(label.name.text)) {
...
function isRangeMacro(state, node) {
    if (typescript_1.default.isCallExpression(node.expression)) {
        const symbol = (0, types_1.getFirstDefinedSymbol)(state, state.getType(node.expression.expression));
...
function transformForOfRangeMacro(state, node) {
    ...
    const [[start, end, step], prereqs] = state.capture(() => (0, ensureTransformOrder_1.ensureTransformOrder)(state, node.expression.arguments));
    luau_ast_1.default.list.pushList(result, prereqs);
    luau_ast_1.default.list.pushList(statements, (0, transformStatementList_1.transformStatementList)(state, (0, getStatements_1.getStatements)(node.statement)));
