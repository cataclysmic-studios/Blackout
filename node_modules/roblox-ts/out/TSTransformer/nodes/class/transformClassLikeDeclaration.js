...
function createRoactBoilerplate(state, node, className, isClassExpression) {
    ...
    const classNameStr = luau_ast_1.default.isIdentifier(className) ? className.name : "Anonymous";
    const right = luau_ast_1.default.create(luau_ast_1.default.SyntaxKind.MethodCallExpression, {
        expression: (0, convertToIndexableExpression_1.convertToIndexableExpression)(extendsExp),
        name: "extend",
        args: luau_ast_1.default.list.make(luau_ast_1.default.string(classNameStr)),
    });
...
function createBoilerplate(state, node, className, isClassExpression) {
    ...
    if (isAbstract && !extendsNode) {
        luau_ast_1.default.list.push(statements, luau_ast_1.default.create(luau_ast_1.default.SyntaxKind.Assignment, {
            left: className,
            operator: "=",
            right: luau_ast_1.default.mixedTable(),
        }));
    }
    else {
...
function extendsMacroClass(state, node) {
    const extendsNode = (0, getExtendsNode_1.getExtendsNode)(node);
    if (extendsNode) {
        const symbol = (0, getOriginalSymbolOfNode_1.getOriginalSymbolOfNode)(state.typeChecker, extendsNode.expression);
        if (symbol) {
            return (symbol === state.services.macroManager.getSymbolOrThrow(TSTransformer_1.SYMBOL_NAMES.ArrayConstructor) ||
                symbol === state.services.macroManager.getSymbolOrThrow(TSTransformer_1.SYMBOL_NAMES.SetConstructor) ||
                symbol === state.services.macroManager.getSymbolOrThrow(TSTransformer_1.SYMBOL_NAMES.MapConstructor) ||
                symbol === state.services.macroManager.getSymbolOrThrow(TSTransformer_1.SYMBOL_NAMES.WeakSetConstructor) ||
                symbol === state.services.macroManager.getSymbolOrThrow(TSTransformer_1.SYMBOL_NAMES.WeakMapConstructor) ||
                symbol === state.services.macroManager.getSymbolOrThrow(TSTransformer_1.SYMBOL_NAMES.ReadonlyMapConstructor) ||
                symbol === state.services.macroManager.getSymbolOrThrow(TSTransformer_1.SYMBOL_NAMES.ReadonlySetConstructor));
        }
    }
    return false;
}
...
function transformClassLikeDeclaration(state, node) {
    ...
    let internalName;
    if (shouldUseInternalName) {
        internalName = node.name ? (0, transformIdentifier_1.transformIdentifierDefined)(state, node.name) : luau_ast_1.default.tempId("class");
    }
    else {
    ...
    const statementsInner = luau_ast_1.default.list.make();
    if (isRoact) {
        luau_ast_1.default.list.pushList(statementsInner, createRoactBoilerplate(state, node, internalName, isClassExpression));
    }
    else {
        luau_ast_1.default.list.pushList(statementsInner, createBoilerplate(state, node, internalName, isClassExpression));
    }
    luau_ast_1.default.list.pushList(statementsInner, (0, transformClassConstructor_1.transformClassConstructor)(state, node, internalName, getConstructor(node)));
    for (const member of node.members) {
        if ((typescript_1.default.isPropertyDeclaration(member) || typescript_1.default.isMethodDeclaration(member)) &&
            ...
            (luau_ast_1.default.isReservedClassField(member.name.text) ||
                (isRoact && luau_ast_1.default.isReservedRoactClassField(member.name.text)))) {
    ...
    for (const method of methods) {
        ...
        luau_ast_1.default.list.pushList(statementsInner, (0, transformMethodDeclaration_1.transformMethodDeclaration)(state, method, { name: "name", value: internalName }));
    }
    ...
    for (const property of staticProperties) {
        luau_ast_1.default.list.pushList(statementsInner, (0, transformPropertyDeclaration_1.transformPropertyDeclaration)(state, property, internalName));
    }
    if (shouldUseInternalName) {
