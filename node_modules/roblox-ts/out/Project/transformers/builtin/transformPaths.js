...
const transformPaths = (context) => (sourceFile) => {
    ...
    function unpathRequireAndAsyncImport(node) {
        ...
        const fileLiteral = typescript_1.default.createLiteral(file);
        return typescript_1.default.updateCall(node, node.expression, node.typeArguments, [fileLiteral]);
    }
    function unpathImportTypeNode(node) {
        ...
        const fileLiteral = typescript_1.default.createLiteral(file);
        const fileArgument = typescript_1.default.updateLiteralTypeNode(argument, fileLiteral);
    ...
    function unpathImportEqualsDeclaration(node) {
        ...
        const fileLiteral = typescript_1.default.createLiteral(file);
        return typescript_1.default.updateExternalModuleReference(node, fileLiteral);
    }
    function unpathImportDeclaration(node) {
        ...
        return node.importClause === importClause || importClause || isDeclarationFile
            ? typescript_1.default.updateImportDeclaration(node, node.decorators, node.modifiers, node.importClause, fileLiteral, undefined)
            : undefined;
    }
    function visitImportClause(node) {
        ...
        return name || namedBindings ? typescript_1.default.updateImportClause(node, name, namedBindings, node.isTypeOnly) : undefined;
    }
    function visitNamedImportBindings(node) {
        if (node.kind === typescript_1.default.SyntaxKind.NamespaceImport) {
            ...
            return elements.some(e => e) ? typescript_1.default.updateNamedImports(node, elements) : undefined;
        }
    }
    ...
    function unpathExportDeclaration(node) {
        ...
        if ((!node.exportClause &&
            ...
            return typescript_1.default.updateExportDeclaration(node, node.decorators, node.modifiers, node.exportClause, fileLiteral, node.isTypeOnly);
        }
        ...
        return node.exportClause === exportClause || exportClause || isDeclarationFile
            ? typescript_1.default.updateExportDeclaration(node, node.decorators, node.modifiers, node.exportClause, fileLiteral, node.isTypeOnly)
            : undefined;
    }
    function visitNamedExports(node) {
        ...
        return elements.some(e => e) ? typescript_1.default.updateNamedExports(node, elements) : undefined;
    }
