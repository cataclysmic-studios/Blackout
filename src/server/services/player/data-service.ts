...
import DefaultPlayerData, {
  PlayerData,
  PlayerDataProfile,
} from "shared/meta/default-player-data";
...
export class PlayerDataService {
  ...
  private gameProfileStore = ProfileService.GetProfileStore<PlayerData>(
    "PlayerData",
    DefaultPlayerData
  );
  ...
  public async loadProfile(userID: number): Promise<Maybe<PlayerDataProfile>> {
    ...
    const profile = this.gameProfileStore.LoadProfileAsync(
      dataKey,
      "ForceLoad"
    );
    ...
    if (profile === undefined)
      return this.playerRemoval.removeDueToBug(
        player,
        KickReason.PlayerProfileUndefined
      );
